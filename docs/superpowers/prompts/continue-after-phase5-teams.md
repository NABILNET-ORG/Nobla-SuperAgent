# Phase 5-Channels — Continuation Prompt (Post-Teams)
...
Continue Phase 5 implementation — building the next batch of messaging channel adapters. WhatsApp, Slack, Signal, and Teams are complete. Pick the next adapter(s) from the priority list below.
...
**Phase 5-Channels — Teams (90 tests):**
- Bot Framework REST API (webhook-only), OAuth2 client_credentials TokenManager with auto-refresh
- JWT validation (RS256 via cached JWKS, security-first reject-when-unavailable)
- Adaptive Cards formatter (headings, code, dividers, quotes, action buttons)
- Inline base64 media (≤256KB) + hero card links (>256KB)
- Keyword commands, mention-only channel policy, conversation reference capture
- Multi-tenant support, TeamsSettings + gateway wiring
...
**Test count: 1,723 total (273 Flutter + 1,450 backend)**
...
| `formatter.py` | Platform formatting → `format_response()` returning `list[FormattedMessage]` or Activity dict |
...
11 remaining platform adapters. Prioritized order:

#### Tier 1 — High Priority
1. **Facebook Messenger** — Send/Receive API, webhook verification (X-Hub-Signature-256, same as WhatsApp), message templates, quick replies, persistent menu
2. **Slack Enterprise Grid** — extend existing Slack adapter with org-level features (Enterprise Grid API, cross-workspace messaging)

#### Tier 2 — Medium Priority
3. **LINE** — Messaging API, Flex Messages (JSON-based rich layouts), rich menus, multicast
4. **Viber** — REST API, keyboard buttons, carousel, broadcast
5. **WeChat** — Official Account API, XML message format, JSAPI ticket verification
6. **Matrix** — Client-Server API (Synapse), room-based, optional E2EE via libolm

#### Tier 3 — Lower Priority
7. **Twilio SMS** — REST API, webhook (X-Twilio-Signature HMAC-SHA1), MMS media
8. **Email (IMAP/SMTP)** — aiosmtplib + aioimaplib, MIME parsing, attachment handling
9. **IRC** — asyncio IRC client, channel/PM modes, CTCP
10. **XMPP/Jabber** — aioxmpp or slixmpp, presence, MUC (multi-user chat)
11. **Google Chat** — Bot API, Cards v2 (similar to Adaptive Cards), service account auth

### Key Files to Reference

- `backend/nobla/channels/base.py` — BaseChannelAdapter ABC (the contract)
- `backend/nobla/channels/teams/` — Best reference for OAuth2 token management, JWT validation, Adaptive Cards rich formatting
...
- `backend/nobla/channels/whatsapp/` — Best reference for webhook-only HTTP adapter with HMAC signature verification
...
- `backend/nobla/config/settings.py` — Settings class (add new settings after TeamsSettings)
- `backend/nobla/gateway/lifespan.py` — `_init_channels()` (wire new adapters after Teams block)
- `backend/tests/test_slack_adapter.py` — Most comprehensive test file (142 tests, good pattern)
- `backend/tests/test_teams_adapter.py` — JWT/OAuth2 pattern reference (90 tests)
...
- `docs/superpowers/specs/2026-03-29-teams-adapter-design.md` — Teams adapter design spec
- `docs/superpowers/plans/2026-03-29-teams-adapter.md` — Teams adapter implementation plan
